...
async function saveScreenshots({ page, trip, waypoints, directory } = {}) {
  ...
  for (const { id, index } of waypoints) {
    ...
    if (url !== page.url()) {
      ...
      if (response.status() >= 400) {
        console.error(`ERROR [Screenshots] Waypoint page ${url} not found`);
    ...
    if (comments) {
      ...
      console.error(`ERROR [Screenshots] No comments for waypoint ${id}`);
    }
  }
  ...
  if (map) {
    ...
    console.error(`ERROR [Screenshots] Map not found`);
  }
}
